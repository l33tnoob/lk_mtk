...
OPTIONS.java_args = "-Xmx8912m" # JVM Args
...
class BlockDifference:
  ...
  def WriteVerifyScript(self, script, output_zip):
    if not self.src:
      script.Print("Image %s will be patched unconditionally." % (self.partition,))
    else:
      src_ranges_str = self.src.care_map.subtract(self.src.clobbered_blocks).to_string_raw()
      tgt_ranges_str = self.tgt.care_map.subtract(self.tgt.clobbered_blocks).to_string_raw()
      if self.check_first_block:
        self._CheckFirstBlock(script)

      script.AppendExtra('if range_sha1("%s", "%s") == "%s" then' %
                         (self.device, src_ranges_str,
                          self.src.TotalSha1()))
      script.Print("Verified %s image..." % (self.partition,))
      script.Comment("---- start making changes here ----")
      self.WriteScript(script, output_zip, progress=0.8)
      script.AppendExtra(('else\n'
                          '  (range_sha1("%s", "%s") == "%s") ||\n'
                          '  abort("%s partition has unexpected contents");\n'
                          'endif;') %
                         (self.device, tgt_ranges_str,
  ...
  def _CheckFirstBlock(self, script):
    ...
    m = sha1()
    for data2 in self.tgt.ReadRangeSet(r):
      m.update(data2)
    m = m.hexdigest()

    script.AppendExtra(('(range_sha1("%s", "%s") == "%s") || (range_sha1("%s", "%s") == "%s") || '
#                        'abort("%s has been remounted R/W; '
# HTC replace this command to ui_print, help for debugging from device screen UI
                        'ui_print("%s has been remounted R/W; '
                        'reflash device to reenable OTA updates");')
                       % (self.device, r.to_string_raw(), h, self.device, r.to_string_raw(), m, self.device))
