...
class EdifyGenerator(object):
  ...
  def AssertCid(self, cid_list):
    """Assert that the cid is the given string."""
    cmd = ('assert(check_cid(' + ',\0'.join([('"%s"') % cid for cid in cid_list]) + ') == "t");')

    self.script.append(self._WordWrap(cmd))

  def AssertMid(self, mid_list):
    """Assert that the mid is the given string."""
    full_cmd = ('assert(check_mid("full", ' + ' ,\0'.join([('"%s"') % mid for mid in mid_list]) + ') == "t");')

    self.script.append(self._WordWrap(full_cmd))
  ...
  def UnpackPackageFile(self, src, dst):
    ...
    destination file."""
    self.script.append('assert(package_extract_file("%s", "%s"));' % (src, dst))
  ...
  def DeleteFiles_HTC(self, file_list):
    """Delete all files in file_list."""
    if not file_list: return
    ...
    for i in file_list:
      i = i.strip('\n')
      if i[-1] != "/": # File
        list_file_only.append(i)
      else: # Directory
        list_folder_only.append(i)

    if len(list_file_only) != 0:
      cmd = "delete(" + ",\0".join(['"%s"' % (i,) for i in list_file_only]) + ");"
      self.script.append(self._WordWrap(cmd))
    if len(list_folder_only) != 0:
      cmd = "delete_recursive(" + ",\0".join(['"%s"' % (i,) for i in list_folder_only]) + ");"
      self.script.append(self._WordWrap(cmd))
  ...
  def FileExistCheck(self, file_path):
    """Check the existence of a file or a directory."""
    self.script.append('fileexist("%s");' % (file_path))
