...
# U61_Nelson_Li: print HTC env variables.
$(info ********************************************************************************)
$(info PRIVATE_RCMS_NAME = $(PRIVATE_RCMS_NAME) )
$(info PRIVATE_RCMS_SKU = $(PRIVATE_RCMS_SKU) )
$(info PRIVATE_BUILD_CHANGELIS = $(PRIVATE_BUILD_CHANGELIS) )
$(info BCMS_SENSE_VERSION = $(BCMS_SENSE_VERSION) )
$(info Htc_Sense_Version = $(Htc_Sense_Version) )
$(info Htc_Extra_Sense_Version = $(Htc_Extra_Sense_Version) )
$(info HTC_DEBUG_FLAG = $(HTC_DEBUG_FLAG) )
$(info HTC_CFLAG_DEBUG = $(HTC_CFLAG_DEBUG) )
$(info HTC_NonDEBUG_FLAG = $(HTC_NonDEBUG_FLAG) )
$(info HTC_DISCLOSE_FLAG = $(HTC_DISCLOSE_FLAG) )
$(info HTC_SENSE_CHECK = $(HTC_SENSE_CHECK) )
$(info HTC_PROTECT_CONFIDENTIAL_FLAG = $(HTC_PROTECT_CONFIDENTIAL_FLAG) )
$(info HTC_HEP_FLAG = $(HTC_HEP_FLAG) )
$(info HTC_Security_DEUBG_FlAG = $(HTC_Security_DEUBG_FlAG) )
$(info HTC_UT_BUILD = $(HTC_UT_BUILD) )
$(info HTC_DASHBORAD_BUILD = $(HTC_DASHBORAD_BUILD) )
$(info HTC_AA_BUILD = $(HTC_AA_BUILD) )
$(info DEFAULT_DEBUG_PERMISSIVE_MODE = $(DEFAULT_DEBUG_PERMISSIVE_MODE) )
$(info ********************************************************************************)
# U61_Nelson_Li: End print HTC env variables.

# Start HTC build check
# check MFG_PATCH_FAIL
ifeq ($(MFG_PATCH_FAIL),1)
$(warning ********************************************************************************)
$(warning *  MFG Patch Fail, Build Stopped.)
$(warning *  Please check the MFG patch.)
$(warning ********************************************************************************)
$(error stopping)
endif

# check APPLY_SIE_ZIP_RESULT
ifeq ($(APPLY_SIE_RESULT),error)
$(warning ********************************************************************************)
$(warning *  Extract sie zip file have problem.)
$(warning *  Please check sie zip file is usable.)
$(warning ********************************************************************************)
$(error stopping)
endif
...
# ifeq ($(HTC_LUNCH_RESULT),error)
# $(warning ********************************************************************************)
# $(warning *  )
# $(warning *   Run export_htcvar_by_conf has problem.)
# $(warning *   Please check device Sense setting exist or not.)
# $(warning * )
# $(warning ********************************************************************************)
# $(error stopping)
# endif
...
# check HTC_LUNCH RESULT
# ifeq ($(HTC_SENSE_CHECK),error)
# $(warning ********************************************************************************)
# $(warning *  )
# $(warning *   Run check_Htc_Sense_Version error.)
# $(warning *   Please check HTC Sense Version.)
# $(warning * )
# $(warning ********************************************************************************)
# $(error stopping)
# endif
...
$(info HTC DEBUG BOOT: Make Command Goals[$(MAKECMDGOALS)])

ifneq ($(filter htcdebugboot, $(MAKECMDGOALS)),)
  $(info HTC DEBUG BOOT: Enable Debugging flags...)

  # Make sure security off
  HTC_PROTECT_CONFIDENTIAL_FLAG := false
endif

# BCC+
BCC_MAKEFILE := device/htc/customize/acc/BCCConfig.mk
BCC_MAKEFILE_EXIST := $(shell if test -e $(BCC_MAKEFILE); then echo yes; else echo no; fi)
#use hard-code path to let BCC recognize
ifeq ($(BCC_MAKEFILE_EXIST), yes)
include device/htc/customize/acc/BCCConfig.mk
else
    $(warning No $(BCC_MAKEFILE) found.)
endif
# BCC-
...
ifneq (,$(user_variant))
  ...
  ifeq ($(user_variant),userdebug)
    ...
    ADDITIONAL_DEFAULT_PROPERTIES += ro.secure=1
    ADDITIONAL_DEFAULT_PROPERTIES += persist.sys.usb.config=adboff
