...
TARGET_STRIP_COMMAND = $(PRIVATE_STRIP) --strip-all $< -o $@ $(TARGET_STRIP_EXTRA)
ifeq ($(HTC_DEBUG_FLAG), DEBUG)
TARGET_STRIP_COMMAND_DEBUG = $(PRIVATE_STRIP) --strip-debug $< -o $@ $(TARGET_STRIP_EXTRA)
else
TARGET_STRIP_COMMAND_DEBUG = $(TARGET_STRIP_COMMAND)
endif

define transform-to-stripped
@mkdir -p $(dir $@)
@echo "target Strip: $(PRIVATE_MODULE) ($@)"
$(hide) $(if $(findstring true,$(PRIVATE_STRIP_DEBUG)), \
	$(TARGET_STRIP_COMMAND_DEBUG), \
...
# Generate CCC Certification.
# U61000 Nelson_Li
define gen-ccc-cert-apk
$(hide) java -jar $(CCC_CERT_JAR) \
    generate-apk $@ $(dir $<)mlappinfo.xml "CN=HTC" $@.crt
$(hide) rm $@
$(hide) mv $@.crt $@
endef
...
# Generate CCC APPID
# U61000 Nelson_Li
define gen-ccc-cert-appid
$(hide) echo [APPID] $(notdir $(shell echo "$(dir $@)" | sed 's;_intermediates/;;')) :`java -jar $(CCC_CERT_JAR) \
    generate-appid $@`
endef

# Generate CCC APPID to out
# U61000 Nelson_Li
define gen-ccc-cert-appid_to_out
$(hide) echo [APPID] $(notdir $(shell echo "$(dir $@)" | sed 's;_intermediates/;;')) :`java -jar $(CCC_CERT_JAR) generate-appid $@` >> $(PRODUCT_OUT)/CCC_APPID.txt
endef
