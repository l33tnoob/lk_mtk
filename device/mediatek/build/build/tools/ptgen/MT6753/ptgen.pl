...
=cut
	...
	$ArgList{DIFFER_PLATFROM_PARTITION_SUPPORT} = $ENV{MTK_DIFFER_PLATFROM_PARTITION_SUPPORT};
    $ArgList{EFUSE_WRITER_SUPPORT} = $ENV{MTK_EFUSE_WRITER_SUPPORT};
    $ArgList{SKU_PARTITION_SUPPORT} = $ENV{MTK_SKU_PARTITION_SUPPORT};
    ...
    #filepath;
    if($ArgList{DIFFER_PLATFROM_PARTITION_SUPPORT} eq "yes")
    {
        $ptgen_location = "device/htc/$ENV{TARGET_PRODUCT}";
    }
    else
    {
    ...
    {
        $Partition_layout_xls = "$ptgen_location/partition_table_$ArgList{PLATFORM}$ArgList{SKU_PARTITION_SUPPORT}.xls";
    ...
    %download_files = (
                       ...
                       srampreld => "sram_preloader.bin",
                       mempreld  => "mem_preloader.bin",
                       lk      => "lk.bin",
                       boot    => "boot.img",
