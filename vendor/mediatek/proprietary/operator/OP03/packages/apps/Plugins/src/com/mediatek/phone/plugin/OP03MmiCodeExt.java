   ...
   public OP03MmiCodeExt(Context context) {
    ...
    public void onMmiDailogShow(Message buttonCallbackMessage) {
        if(buttonCallbackMessage == null){
            Xlog.d(LOG_TAG, "ussd does not dimiss as buttonCallbackMessage is null");
            return;
        }
        ...
        Xlog.d(LOG_TAG, "ussd dismisses in 5 minutes if network does not dismiss it onMmiDailogShow");
    }
    public boolean showUssdInteractionDialog(Phone phone , EditText inputText) {
        if (inputText.length() <= MAX_USSD_LEN) {
            ...
            Xlog.d(LOG_TAG, "send response to network when input length is less than MAX_USSD_LEN");
            return true;
        }
        else {
        Xlog.d(LOG_TAG, "raise toast when input length is greater than MAX_USSD_LEN");
        return false;
    }
    }
    ...
    public void configBeforeMmiDialogShow(MmiCode mmiCode) {
        ...
        Xlog.d(LOG_TAG, "ussd mUserInit state =" + mUserInit);
    }

    public boolean skipPlayingUssdTone() {
        Xlog.d(LOG_TAG, "ussd beeps when mUserInit is 0 if 1 it is Userinitiated and does not beep " + mUserInit);
        return mUserInit;
   }
}
