...
public abstract class AbstractBackupActivity extends CheckedListActivity implements OnCheckedCountChangedListener, OnUnCheckedChangedListener {
    ...
    public void onClick(View v) {
        ...
        case R.id.backup_bt_backcup:
            ...
            /*String path = StorageSettingsActivity.getCurrentPath(this);
            Log.i(CLASS_TAG, "mButton---Backup clicked path = "+ path);
            if (path != null && SDCardUtils.getAvailableSize(path) <= 512) {
    ...
    protected Dialog onCreateDialog(final int id, final Bundle args) {
        ...
        case DialogID.DLG_CHANGE_NOTICE:
            dialog = createDeviceInfoDialog(R.string.change_phone_info,Constants.NOSDCARD_CHANGE_NOTICE,R.array.change_phone_info);
            break;
        case DialogID.DLG_SDCARD_UNMOUNT_BACKUP:
            dialog = createDeviceInfoDialog(R.string.sdcard_unmount_backup,Constants.SDCARD_UNMOUNT_BACKUP,R.array.sdcard_unmount_backup);
            break;
        case DialogID.DLG_RESET_BACKUP_PATH:
            dialog = new AlertDialog.Builder(this).setTitle(R.string.remind).setMessage(R.string.reset_path_info)
                    .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            Intent intent = new Intent(getApplicationContext(),StorageSettingsActivity.class);
            ...
            dialog = new AlertDialog.Builder(this).setTitle(R.string.remind).setView(view)
                   .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int whichButton) {
                            ...
                            StorageSettingsActivity.setNoticeStatus(getApplicationContext(),mChecked , Constants.NOTICE_BACKUP_PATH);
                            switch (mRequestCode) {
    ...
    private Dialog createDeviceInfoDialog(int changePhoneInfo, final String key, final int changePhoneInfoArray) {
        LayoutInflater factory = LayoutInflater.from(this);
        final View view = factory.inflate(R.layout.dialog_change_notice, null);
        TextView text = (TextView)view.findViewById(R.id.change_info);
        ...
        Dialog dialog = new AlertDialog.Builder(this).setTitle(R.string.change_phone_notice).setView(view)
               .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int whichButton) {
                        Log.e(CLASS_TAG, "DLG_CHANGE_NOTICE: mChecked = "+mChecked +" key == " + key);
                        StorageSettingsActivity.setNoticeStatus(getApplicationContext(),mChecked , key);
                   ...
                   @Override
                   public void onClick(DialogInterface dialog, int which) {
                       String[] mData = getApplicationContext().getResources().getStringArray(changePhoneInfoArray);
                        Intent intent = new Intent(getApplicationContext(),DeviceChangedInfo.class);
...
//                    mBackupService.reset();
    ...
    protected boolean checkedStartBackup() {
        ...
        Log.i(CLASS_TAG, "checkedStartBackup : getPathIndexKey = "+ StorageSettingsActivity.getPathIndexKey(this));
        ...
        } else if (path != null && StorageSettingsActivity.getPathIndexKey(this).equals(Constants.CUSTOMIZE_STOTAGE)) {
            if(!SDCardUtils.checkedPath(path)) {
                Log.e(CLASS_TAG, "the path is sdcard  , but not exists,must be set!");
                showDialog(DialogID.DLG_RESET_BACKUP_PATH);
                return false;
                ...
                if (SDCardUtils.isSupprotSDcard() && SDCardUtils.getSDCardDataPath() == null
                        && path.contains(SDCardUtils.getPhoneStoragePath())) {
                    if (!StorageSettingsActivity.getNoticeStatus(this, Constants.SDCARD_UNMOUNT_BACKUP)) {
                        Log.i(CLASS_TAG,
