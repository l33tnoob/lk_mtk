...
my $chk_oemkey_tool = "$cfg_dir/check_VERIFIED_BOOT_IMG_AUTH_KEY.py";
# HTC_CSP_END
my $BUILD_UBOOT = "no";
$ENV{PROJECT}=$prj;
my $secro_type = "GMP";
...
# File Check
##########################################################
my @imgs_need_sign_raw = ("lk.bin", "logo.bin", "secro.img", "boot.img", "recovery.img","hosd.img");
...
# HTC_CSP_START, #21487(Dybert_Wang), Add
print "<<== HTC custimization sign key process start ==>>\n";
system("python $chk_oemkey_tool") == 0 or die "hTC check $oemkey fail";
print "<<== HTC custimization sign key process  end  ==>>\n";
# HTC_CSP_END
...
foreach my $img (@imgs_need_sign_raw) {
    ...
    $signed_verified_img =~ s/\./_verified\./;
    # HTC_CSP_END
	my $sig_img = $img;
	$sig_img =~ s/\.bin/\.sig/;
	$sig_img =~ s/\.img/\.sig/;
