...
#include <hardware_legacy/AudioMTKPolicyInterface.h>
...
extern "C" {
    ...
    struct legacy_audio_policy {
        struct audio_policy_mtk policy;

        void *service;
        struct audio_policy_service_ops *aps_ops;
        AudioPolicyCompatClient *service_client;
        AudioMTKPolicyInterface *apm;
    };
...
//Ainge: Temporoary Modification for L PDK build pass
                                ...
                                struct audio_policy **ap)
    {
        ...
        lap->policy.set_policy_parameters = ap_set_policy_parameters; // MTK_AOSP_ENHANCEMENT
        ...
        lap->apm = createAudioMTKPolicyManager(lap->service_client);
        if (!lap->apm) {
            ret = -ENOMEM;
            goto err_create_apm;
        }
...
err_new_compat_client:
                                 ...
                                 struct audio_policy *ap)
    {
        ...
        if (lap->apm)
            destroyAudioMTKPolicyManager(lap->apm);
        if (lap->service_client)
            delete lap->service_client;
        free(lap);
        return 0;
