                                                   ...
                                                   const audio_offload_info_t *offloadInfo)
{
    ...
    if (profile != NULL)
    {
        ...
        if(profile->mSupportedDevices & AUDIO_DEVICE_OUT_ALL_A2DP)
        {
            ...
            {   // if pcm is playing, reject offoad
                ...
                ALOGD("AudioMTKPolicyManager::getOutput offload getParameter keyLosslessBTPcmPlaying=%s", AudioParameter::keyLosslessBTPcmPlaying, ret.string());
                if (ret==String8("yes")){
        ...
        ALOGD("getOutput() [mpClientInterface->openOutput] device %d, stream %d, samplingRate %d, format %x, channelMask %x, flags %x",
              outputDesc->mDevice,stream, samplingRate, format, channelMask, flags);
        ...
        if( outputDesc->mDevice == AUDIO_DEVICE_OUT_AUX_DIGITAL)
        {
            channelMask = outputDesc->mChannelMask;
            ALOGD("ignore channel mask differenct for HDMI, channelMask 0x%x", channelMask);
        }
        #endif
    ...
    if (audio_is_linear_pcm((audio_format_t)format))
    {
        ...
        if(output==getA2dpOutput()){
            ...
            ALOGD("AudioMTKPolicyManager::getOutput offload getParameter %s=%s", AudioParameter::key4BTOffloadPlaying, ret.string());
            if (ret==String8("yes")){
...
bool AudioMTKPolicyManager::isOffloadSupported(const audio_offload_info_t &offloadInfo)
{
    ...
    if(profile != NULL && a2dp_handle!=0){
        String8 ret = mpClientInterface->getParameters(a2dp_handle,String8(AudioParameter::keyLosslessBTStatus));
        ALOGD("isOffloadSupported() getParameter %s=%s", AudioParameter::keyLosslessBTStatus,ret.string());
        if (ret!=String8("yes")){
            return false;
        }
    }
...
#ifdef MTK_AUDIO//Add support FM headphone -> WFD/A2DP
        && !((device & (AUDIO_DEVICE_OUT_REMOTE_SUBMIX | AUDIO_DEVICE_OUT_ALL_A2DP)) && isStreamActive(AudioSystem::FM))
#endif
       )
    {
        return 0;
                                           ...
                                           audio_devices_t device)
{
    ...
    {
        volume = computeCustomVolume(stream, index, output, device);
    }
    else
    {
...
#else
    checkAndSetAnalogGain(stream, index, output, device, delayMs, force);
#endif
...
#ifdef MTK_AUDIO
#ifdef MTK_AUDIO_GAIN_TABLE
float AudioMTKPolicyManager::computeCustomVolume(int stream, int index, audio_io_handle_t output, audio_devices_t device)
{
    float volume = 1.0;
    ALOGD("+computeCustomVolume stream %d, index %d, device 0x%x", stream, index, device);
    ...
    ALOGD("-computeCustomVolume customGain 0x%x, volume %f", customGain, volume);
    return volume;
}

int AudioMTKPolicyManager::selectActiveStream(int requestStream)
{
    ...
    else if (isStreamActive(AUDIO_STREAM_FM))
    {
        activeStream = AUDIO_STREAM_FM;
    }
    else if (isStreamActive(AUDIO_STREAM_MATV))
    {
        activeStream = AUDIO_STREAM_MATV;
    }
...
status_t AudioMTKPolicyManager::checkAndSetAnalogGain(int stream, int index, audio_io_handle_t output, audio_devices_t device,
                                                      int delayMs, bool force)
{
    ...
    AudioOutputDescriptor *outputDesc = mOutputs.valueFor(output);
    int activeStream = selectActiveStream(stream);
...
#ifdef MTK_AUDIO_GAIN_TABLE
    android::GetAudioGainTableParamFromNV(&mCustomVolume);
#else
...
status_t AudioMTKPolicyManager::AdjustMasterVolume(int stream, int index, audio_io_handle_t output, AudioOutputDescriptor *outputDesc, unsigned int condition);
{
    return INVALID_OPERATION;
}
...
float AudioMTKPolicyManager::computeCustomVolume(int stream, float &vol, audio_io_handle_t output, audio_devices_t device)
{
    return 0.0;
}
void AudioMTKPolicyManager::LoadCustomVolume(void)
{
    return;
}
int AudioMTKPolicyManager::GetStreamMaxLevels(int streamtype)
