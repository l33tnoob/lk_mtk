...
char *SpeechMessengerEVDO::GetEvdoDeviceName()
{
    ...
    dlclose(dlHandle);
    snprintf(dev, 64, "%s", pcvdev);
    ALOGD("GetEvdoDeviceName, %s", pcvdev);
    ALOGD("GetEvdoDeviceName, %s", dev);

    free(pcvdev);
#endif
    char *dev0 = NULL;
    dev0 = viatelAdjustDevicePathFromProperty(VIATEL_CHANNEL_PCV);
    dev0 = "/dev/ttySDIO0";   //JT temp: get device node name fail from 64 bit libc2kutils.so
    ALOGD("GetEvdoDeviceName %s", dev0);
...
void SpeechMessengerEVDO::OpenMuxdDeviceUntilReady()
{
    ...
    while (fHdl < 0)
    {
        ...
        char *DEVICE_NAME = GetEvdoDeviceName();
...
status_t SpeechMessengerEVDO::Initial()
{
    ...
    if (fHdl < 0)
    {
        ALOGE("%s(), open(%s) fail, fHdl = %d, errno=%d, return", __FUNCTION__, DEVICE_NAME, fHdl, errno);
