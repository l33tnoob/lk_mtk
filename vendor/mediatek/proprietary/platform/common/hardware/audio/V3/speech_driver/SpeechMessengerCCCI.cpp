...
int32_t SpeechMessengerCCCI::SendMessage(const ccci_buff_t &ccci_buff)
{
    ...
    {
        ALOGE("%s(), modem_status(%c) != MODEM_STATUS_READY, errno: %d", __FUNCTION__, modem_status, errno);
...
//    char *p_data_curIdx = (char *)ul_ring_buf.pRead + 6; // size(unsigned short)
//    char *p_data_totalIdx  = (char *)ul_ring_buf.pRead + 8; // size(unsigned short)
    ...
    if (ul_ring_buf.pWrite >= mM2AShareBufEnd)
    {
        ...
        memcpy((char *)ccci_buff.payload, (char *)ul_ring_buf.pRead,  size_copy1);
        memcpy((char *)ccci_buff.payload, (char *)ul_ring_buf.pRead + size_copy1, size_copy2);
    }
    else
    {
        memcpy((char *)ccci_buff.payload, (char *)ul_ring_buf.pRead,  BytesCopied);
    }
