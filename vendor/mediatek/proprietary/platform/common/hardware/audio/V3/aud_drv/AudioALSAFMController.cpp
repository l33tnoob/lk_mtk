...
//HTC_AUD_ADD
        // set default 48000 Hz to avoid Ringtone or Playback during FM will cause sampleRate not match
...
//HTC_AUD_START    For FM Speaker mode set I2S Connection
            if (AUDIO_DEVICE_OUT_SPEAKER == output_device) {
                ALOGD("FM Speaker mode, setConnection for FM speaker mode");
                if (mixer_ctl_set_enum_by_string(mixer_get_ctl_by_name(AudioALSADriverUtility::getInstance()->getMixer(),
                                                                       "Audio_FM_I2S_Connection"), "On"))
                {
                    ALOGE("Error: Audio FM I2S Connection invalid value");
                }
            }
//HTC_AUD_END
...
status_t AudioALSAFMController::routing(const audio_devices_t pre_device, const audio_devices_t new_device)
{
    ...
    {
#if 0 //removed by eleven_xiang to fix fm swithing from speaker to headset too long, improve user experience.
...
//HTC_AUD_START    For FM Speaker/Normal mode set I2S Connection
    if (AUDIO_DEVICE_OUT_SPEAKER == new_device) {
        ALOGD("FM Speaker mode, setConnection for FM speaker mode");
        if (mixer_ctl_set_enum_by_string(mixer_get_ctl_by_name(AudioALSADriverUtility::getInstance()->getMixer(), "Audio_FM_I2S_Connection"), "On"))
        {
            ALOGE("Error: Audio FM I2S Connection invalid value");
        }
    } else {
        ALOGD("FM Normal mode, setConnection for FM Normal mode");
        if (mixer_ctl_set_enum_by_string(mixer_get_ctl_by_name(AudioALSADriverUtility::getInstance()->getMixer(), "Audio_FM_I2S_Connection"), "Off"))
        {
            ALOGE("Error: Audio FM I2S Connection invalid value");
        }
    }
