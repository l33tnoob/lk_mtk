...
void META_MATV_OP(FT_MATV_REQ *p_req)
{
    ...
    ALOGD("[FTT_Drv:] FT_MATV, OP=%d ", p_req->type);
    switch(p_req->type) 
    {
        case FT_MATV_OP_POWER_ON:
            if(matv_power_on == KAL_FALSE)
            {
            	ALOGD("[FTT_Drv:] matv need init ");
                matv_power_on = matv_init();
                if(KAL_TRUE == matv_power_on)
                {
                	ALOGD("[FTT_Drv:] matv_init succeeded!");
                    matv_cnf.status = META_SUCCESS;
                }
                else
                {
					ALOGD("[FTT_Drv:] matv_init failed!");
                    matv_cnf.status = META_FAILED;
                }
            }
            else
		...
		case FT_MATV_OP_GET_CHIPNAME:
		{
			...
			matv_cnf.status = META_SUCCESS;
			ALOGD("[FTT_Drv:] FT_MATV_OP_GET_CHIPNAME id %c%c%c%c%c%c succeeded! ", matv_cnf.result.chipname[0], matv_cnf.result.chipname[1],
