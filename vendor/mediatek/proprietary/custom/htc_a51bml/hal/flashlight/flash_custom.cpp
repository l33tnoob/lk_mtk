...
void cust_getFlashITab2(int sensorDev, short* ITab2)
{
    if(sensorDev==DUAL_CAMERA_MAIN_SENSOR)
	{
		if(g_mainId==1)
		{
			XLOGD("cust_getFlashITab2 devid main id1");
			cust_getFlashITab2_main(ITab2);
		}
		else
		{
			XLOGD("cust_getFlashITab2 devid main id2");
	...
	else
	{
		if(g_subId==1)
		{
			XLOGD("cust_getFlashITab2 devid sub id1");
			cust_getFlashITab2_sub(ITab2);
		}
		else
		{
			XLOGD("cust_getFlashITab2 devid sub id2");
