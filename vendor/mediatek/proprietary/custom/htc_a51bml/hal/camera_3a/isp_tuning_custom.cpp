...
{
#define NORMAL_SWNR_ENC_ENABLE_ISO_THRESHOLD (750) // enable if ISO >= THRESHOLD
    return NORMAL_SWNR_ENC_ENABLE_ISO_THRESHOLD;
}

MUINT32 get_MFB_SWNR_ENC_enable_ISO_threshold()
{
#define MFB_SWNR_ENC_ENABLE_ISO_THRESHOLD (750) // enable if ISO >= THRESHOLD
...
{
/* HTC_ADD  interpoloation of OBC */
#define GET_INTERPOLATION_RATIO(ct, s, e) (1.0 - ((ct) - (s))/((e) - (s)))
/* Return v1 * ratio + v2 * ( 1.0 - ratio ) */
#define LINEAR_INTERPOLATION(v1, v2, ratio) \
  ((v2) + ((ratio) * ((v1) - (v2))))

    static ISP_NVRAM_OBC_T obc1;
    static ISP_NVRAM_OBC_T obc2;
    MINT32 trigger_iso_th1 = 200;
    MINT32 trigger_iso_th2 = 1200;
    MINT32 trigger_LV = 9999;

    switch (getSensorID()){
        case 0x2c30:
            trigger_iso_th1 = 1300;
            trigger_iso_th2 = 1500;
            trigger_LV = 10;
            break;
        case 0x4688:
            trigger_iso_th1 = 799;
    ...
    switch( rCamInfo.eSensorMode)
    {
        case ESensorMode_Preview:
            if (rCamInfo.eIspProfile == EIspProfile_Video)
            {
                obc1 = rIspRegMgr.getOBC(10);
                obc2 = rIspRegMgr.getOBC(11);
            }
            else // if (rCamInfo.eIspProfile == EIspProfile_Preview)
            {
                obc1 = rIspRegMgr.getOBC(0);
                obc2 = rIspRegMgr.getOBC(1);
            }
            break;
         case ESensorMode_Capture:
            obc1 = rIspRegMgr.getOBC(2);
            obc2 = rIspRegMgr.getOBC(3);
            break;
         case ESensorMode_Video:
            obc1 = rIspRegMgr.getOBC(4);
            obc2 = rIspRegMgr.getOBC(5);
            break;
         case ESensorMode_SlimVideo1:
            obc1 = rIspRegMgr.getOBC(6);
            obc2 = rIspRegMgr.getOBC(7);
            break;
         case ESensorMode_SlimVideo2:
            obc1 = rIspRegMgr.getOBC(8);
            obc2 = rIspRegMgr.getOBC(9);
            break;
        default:
            obc1 = rIspRegMgr.getOBC(0);
            obc2 = rIspRegMgr.getOBC(1);
    ...
    if(i4Iso >=  trigger_iso_th2 && rCamInfo.rAEInfo.i4LightValue_x10 < trigger_LV)
        rOBC = obc2;
    else if (i4Iso <=  trigger_iso_th2 && i4Iso >=  trigger_iso_th1 && rCamInfo.rAEInfo.i4LightValue_x10 < trigger_LV){
         ratio = GET_INTERPOLATION_RATIO(float(i4Iso), float(trigger_iso_th1), float(trigger_iso_th2));
         rOBC.offst0.val = LINEAR_INTERPOLATION(obc1.offst0.val, obc2.offst0.val, ratio);
         rOBC.offst1.val = LINEAR_INTERPOLATION(obc1.offst1.val, obc2.offst1.val, ratio);
         rOBC.offst2.val = LINEAR_INTERPOLATION(obc1.offst2.val, obc2.offst2.val, ratio);
         rOBC.offst3.val = LINEAR_INTERPOLATION(obc1.offst3.val, obc2.offst3.val, ratio);
         rOBC.gain0.val = LINEAR_INTERPOLATION(obc1.gain0.val, obc2.gain0.val, ratio);
         rOBC.gain1.val = LINEAR_INTERPOLATION(obc1.gain1.val, obc2.gain1.val, ratio);
         rOBC.gain2.val = LINEAR_INTERPOLATION(obc1.gain2.val, obc2.gain2.val, ratio);
         rOBC.gain3.val = LINEAR_INTERPOLATION(obc1.gain3.val, obc2.gain3.val, ratio);
    }else
         rOBC = obc1;
    /* HTC_END */
...
#define LERP(x, lo_x, lo_y, hi_x, hi_y)\
    ...
    static  MINT32 rto_default[10] =
    {24, 22, 20, 18, 16, 14, 12, 10, 8, 6}; //Tower modify for iso1600 Noise 2014-12-26 

    static  MINT32 rto_ov4688[10] = {32, 32, 30, 30, 28, 24, 20, 0, 0, 0};
    static  MINT32 rto_imx214[10] = {32, 30, 28, 24, 20, 10, 0, 0, 0, 0};
