...
MVOID getHDRExpSetting(const HDRExpSettingInputParam_T& rInput, HDRExpSettingOutputParam_T& rOutput, MUINT cam_id)
{
...
// HTC_START
    if (cam_id == 1) { // FrontCam
        dfTargetTopEV	 = 1.5;
        dfSafeTargetTopEV = 0.5;
    }
// HTC_END
...
//// modify ++ /////////////////////////////
    ...
    else //if(rInput.u4AOEMode!=1) //for normal mode (non-VHDR mode) //rInput.u4AOEMode==0 : standard bracketing, rInput.u4AOEMode==2 : adaptive bracketing
	{
        ...
        */
		...
		if(u4AOEMode==0 && rOutput.u4OutputFrameNum == 3) //Č{ -2, 0, +2 } for standard bracketing                                                                             
		{						
		...
		else if(u4AOEMode==0 && rOutput.u4OutputFrameNum == 2) //Č{ -1, +2 } for standard bracketing                                                                                  
		{				
			// decide ExpTime and Sensor Gain for all images
		...
		else if(u4AOEMode==2 && rOutput.u4OutputFrameNum == 3) //Č{ SE, NE, LE } for adaptive bracketing 
		{		   							       
		...
		else if(u4AOEMode==2 && rOutput.u4OutputFrameNum == 2) //Č{ LE, SE } for adaptive bracketing        
		{							       
			if(dfTargetTopEV<=0.667)
			{
		...
		XLOGD("SWHDR (PLineAETable.pCurrentTable->sPlineTable[u4TargetTopIdx].u4Eposuretime) = %d\n",(PLineAETable.pCurrentTable->sPlineTable[u4TargetTopIdx].u4Eposuretime));
        XLOGD("SWHDR (PLineAETable.pCurrentTable->sPlineTable[u4TargetNormalIdx].u4Eposuretime) = %d\n",(PLineAETable.pCurrentTable->sPlineTable[u4TargetNormalIdx].u4Eposuretime));	
        XLOGD("SWHDR (PLineAETable.pCurrentTable->sPlineTable[u4TargetBottomIdx].u4Eposuretime) = %d\n",(PLineAETable.pCurrentTable->sPlineTable[u4TargetBottomIdx].u4Eposuretime));	
		...
		else if(rOutput.u4OutputFrameNum == 2)
		{
		    ...
		    {
			    XLOGD("SWHDR Error calculation of GainDiff = %f\n",dfGainDiff[0]);
	        }
