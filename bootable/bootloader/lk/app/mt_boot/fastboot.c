...
#if _MAKE_HTC_LK
	if (0 == htc_fastboot_cmd_policy_check(prefix)) return;
#endif
...
#if _MAKE_HTC_LK
	if (0 == htc_fastboot_getvar_policy_check(name)) return;
#endif
...
#ifdef MTK_BQ24296_SUPPORT
		ret = event_wait_timeout(&txn_done, 10000);
...
void fastboot_prhex(unsigned char *ptr, unsigned long data_len)
{
	int line_count = 0, bit_count = 0;
	char response[MAX_RSP_SIZE];

	while(data_len) {
		for(line_count = 0; line_count < 32; line_count++, bit_count++) {
			snprintf(response, MAX_RSP_SIZE, "INFO"
...
#if _MAKE_HTC_LK
int fastboot_write(const unsigned char *data, int size)
{
        if (fastboot_state != STATE_COMMAND){
                return -1;
		}
...
static int fastboot_handler(void *arg)
{
	...
	for (;;) {
#ifdef MTK_BQ24296_SUPPORT
		ret = event_wait_timeout(&usb_online, 10000);
		if (ret == ERR_TIMED_OUT) {
			bq24296_charging_enable(KAL_TRUE);
			continue;
		}
...
#endif
    if (get_bat_sense_volt(1) > 3550)
        fastboot_publish("battery-status", "good");
    else
        fastboot_publish("battery-status", "low");
#ifdef MTK_JTAG_SWITCH_SUPPORT
    fastboot_register("oem ap_jtag",cmd_oem_ap_jtag, TRUE);
#endif

#if _MAKE_HTC_LK
		htc_oem_command_register();
		htc_dump_command_register();
#endif
