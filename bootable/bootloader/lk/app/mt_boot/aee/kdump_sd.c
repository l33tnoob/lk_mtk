...
static int kdump_file_output(const struct mrdump_control_block *mrdump_cb, uint32_t total_dump_size, struct mrdump_dev *mrdump_dev)
{
    ...
    if (OpenDumpFile_sd(file_handle)) {
        ...
        if (ok) {
            ...
            mrdump_status_ok("%s:%s\nMODE:%s\nFILENAME:%s\n", mrdump_dev->name, "vfat", mrdump_mode2string(mrdump_cb->crash_record.reboot_mode), "CEDump.kdmp");
        }
    }
...
int kdump_emmc_output(const struct mrdump_control_block *kparams, uint32_t total_dump_size)
{
    ...
    case MRDUMP_FS_EXT4:
        ...
        ret = kdump_ext4_output(kparams, total_dump_size, mrdump_dev_emmc_ext4());
        break;
    case MRDUMP_FS_VFAT:
        voprintf_info("Output to VFAT Partition\n");
        ret = kdump_file_output(kparams, total_dump_size, mrdump_dev_emmc_vfat());
        break;
    case MRDUMP_FS_NULL:
    default:
        voprintf_info("Performance Tests...\n");
        ret = kdump_null_output(kparams, total_dump_size);
        break;
