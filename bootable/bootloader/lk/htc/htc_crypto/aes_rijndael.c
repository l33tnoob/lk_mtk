 ...
 */
int htc_rijndaelSetupEncrypt(u32 *rk, const u8 *key, int keybits)
{
  int i = 0;
  u32 temp;
 ...
 * @return the number of rounds for the given cipher key size.
 */
int htc_rijndaelSetupDecrypt(u32 *rk, const u8 *key, int keybits)
{
  int nrounds, i, j;
  u32 temp;

  /* expand the cipher key: */
  nrounds = htc_rijndaelSetupEncrypt(rk, key, keybits);
...
void htc_rijndaelEncrypt(const u32 *rk, int nrounds, const u8 plaintext[AES_BLOCK_SIZE],
...
void htc_rijndaelDecrypt(const u32 *rk, int nrounds, const u8 ciphertext[AES_BLOCK_SIZE],
...
void htc_AES_cbc_crypto(const unsigned char *in, unsigned char *out, unsigned long length,
	...
	unsigned char tmp[AES_BLOCK_SIZE];
        unsigned long rk[RKLENGTH(key_size)];
        int nrounds;

	if (crypto == AES_ENCRYPT) {
                nrounds = htc_rijndaelSetupEncrypt(rk, szKey, key_size);
		while (length >= AES_BLOCK_SIZE) {
			for(i=0; i < AES_BLOCK_SIZE; ++i)
				out[i] = in[i] ^ ivec[i];
                        htc_rijndaelEncrypt(rk, nrounds, out, out);
			ivec = out;
		...
		if (length) {
			for(i=0; i < length; ++i)
				out[i] = in[i] ^ ivec[i];
			for(i=length; i < AES_BLOCK_SIZE; ++i)
				out[i] = ivec[i];
                        htc_rijndaelEncrypt(rk, nrounds, out, out);
			ivec = out;
		}
	} else if (in != out) {
                nrounds = htc_rijndaelSetupDecrypt(rk, szKey, key_size);
		while (length >= AES_BLOCK_SIZE) {
...
void htc_AES_ctr_crypto(const unsigned char *in, unsigned char *out, unsigned long length,
	...
	unsigned char tmp[AES_BLOCK_SIZE];
        ...
        if (in != out){
                nrounds = htc_rijndaelSetupEncrypt(rk, szKey, key_size);
                while (length >= AES_BLOCK_SIZE) {
                        htc_rijndaelEncrypt(rk, nrounds, ivec, out);
                        for(i=0; i < AES_BLOCK_SIZE; ++i)
                                out[i] ^= in[i];
                        htc_AES_ctr_inc(ivec);
                        length -= AES_BLOCK_SIZE;
                        in += AES_BLOCK_SIZE;
                        out += AES_BLOCK_SIZE;
                }
                if (length) {
                        htc_rijndaelEncrypt(rk, nrounds, ivec, tmp);
                        for(i=0; i < length; ++i)
                                out[i] = in[i] ^ tmp[i];
                        htc_AES_ctr_inc(ivec);
                }
                ...
                nrounds = htc_rijndaelSetupEncrypt(rk, szKey, key_size);
                while (length >= AES_BLOCK_SIZE) {
		        memcpy(tmp, ivec, AES_BLOCK_SIZE);
                        htc_rijndaelEncrypt(rk, nrounds, ivec, ivec);
                        for(i=0; i < AES_BLOCK_SIZE; ++i)
                                out[i] = in[i] ^ ivec[i];
		        memcpy(ivec, tmp, AES_BLOCK_SIZE);
                        htc_AES_ctr_inc(ivec);
...
void htc_AES_ctr_inc(unsigned char *counter) {
        ...
        c = counter[0] | (counter[1] << 8) | (counter[2] << 16) | (counter[3] << 24);
        c++;
        counter[3] = c >> 24;
        counter[2] = c >> 16;
        counter[1] = c >> 8;
