...
/*================   brightness_set_pmic   ====================
 *
 *      enum mt65xx_led_pmic pmic_type: select the LED to use.
 *          MT65XX_LED_PMIC_NLED_ISINK0 = amber led
 *          MT65XX_LED_PMIC_NLED_ISINK1 = green led
 *      enum led_brightness level: set brightness and blink mode
 *          there is 4 mode can be set
 *          LED_OFF,
 *          LED_FULL,
 *          LED_BLINK_1,
 *          LED_BLINK_2
...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, enum led_brightness level)
{
	...
	switch(level){
		case LED_BLINK1:
			led_duty = 32 * 64 / (64 + 1936);
			led_fsel = ISINK_05HZ;
			break;
		case LED_BLINK2:
			led_duty = 32 * 1000 / (1000 + 1000);
			led_fsel = ISINK_05HZ;
			break;
		default:
			led_duty = 15;
	...
	if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK0)
	{
		...
		pmic_set_register_value(PMIC_ISINK_DIM0_DUTY,led_duty);
		pmic_set_register_value(PMIC_ISINK_DIM0_FSEL,led_fsel);//1KHz
		if (level) 
		{
			pmic_set_register_value(PMIC_RG_DRV_32K_CK_PDN,0x0); // Disable power down
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
	{
		...
		pmic_set_register_value(PMIC_ISINK_DIM1_DUTY,led_duty);
		pmic_set_register_value(PMIC_ISINK_DIM1_FSEL,led_fsel);//1KHz
		if (level) 
		{
			pmic_set_register_value(PMIC_RG_DRV_32K_CK_PDN,0x0); // Disable power down
