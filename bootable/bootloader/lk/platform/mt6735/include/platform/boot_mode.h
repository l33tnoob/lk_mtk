...
#if _MAKE_HTC_LK
  u8 htc_pl_version[16];
  u8 htc_pl_commit_id[16];
  // HTC_CSP_START, #21487(Dybert_Wang), Add
  u32 meid[ME_IDENTITY_LEN>>2];
  u8 htc_sign_key_id[16];
  // HTC_CSP_END
#endif
} BOOT_ARGUMENT;
...
// HTC_CSP_START, #21487(Dybert_Wang), Add
#if _MAKE_HTC_LK
#define BOOT_TAG_PL_INFO      0x88610013

struct boot_tag_pl_info {
    u8 htc_pl_version[16];
    u8 htc_pl_commit_id[16];
    u32 meid[ME_IDENTITY_LEN>>2];
    u8 htc_sign_key_id[16];
...
// HTC_CSP_START, #21487(Dybert_Wang), Add
#if _MAKE_HTC_LK
	struct boot_tag_pl_info htc_pl_info;
#endif
// HTC_CSP_END
    } u;
};
