...
int mboot_common_load_logo(unsigned long logo_addr, char* filename)
{
	...
	dprintf(CRITICAL, "zzytest, mboot_common_load_logo begin\n");
#if (CONFIG_COMMANDS & CFG_CMD_FAT)
...
#if _MAKE_HTC_LK
/**********************************************************
 * Routine: mboot_android_check_hosdimg_hdr
 *
 * Description: this function is called to
 *				(1) 'read' the header of boot image from nand flash
 *				(2) 'parse' the header of boot image to obtain
 ...
 *
 **********************************************************/
static int mboot_android_check_hosdimg_hdr(part_dev_t *dev, char *part_name, boot_img_hdr *boot_hdr)
{
	long len;
#ifdef MTK_EMMC_SUPPORT
	u64 addr;
...
#endif
	if (len < 0) {
		dprintf(CRITICAL, "[%s] %s hosd image header read error. LINE: %d\n", MODULE_NAME, part_name, __LINE__);
		return -1;
	}
	...
	boot_hdr->magic[7] = '\0';
	dprintf(CRITICAL, "[%s] Android Partition Name                : %s\n"    , MODULE_NAME, part_name);
	dprintf(CRITICAL, "[%s] Android Hosd IMG Hdr - Magic          : %s\n"    , MODULE_NAME, boot_hdr->magic);
	dprintf(CRITICAL, "[%s] Android Hosd IMG Hdr - Kernel Size    : 0x%08X\n", MODULE_NAME, boot_hdr->kernel_size);
	dprintf(CRITICAL, "[%s] Android Hosd IMG Hdr - Kernel Address : 0x%08X\n", MODULE_NAME, boot_hdr->kernel_addr);
	dprintf(CRITICAL, "[%s] Android Hosd IMG Hdr - Rootfs Size    : 0x%08X\n", MODULE_NAME, boot_hdr->ramdisk_size);
	dprintf(CRITICAL, "[%s] Android Hosd IMG Hdr - Rootfs Address : 0x%08X\n", MODULE_NAME, boot_hdr->ramdisk_addr);
	dprintf(CRITICAL, "[%s] Android Hosd IMG Hdr - Tags Address   : 0x%08X\n", MODULE_NAME, boot_hdr->tags_addr);
	dprintf(CRITICAL, "[%s] Android Hosd IMG Hdr - Page Size      : 0x%08X\n", MODULE_NAME, boot_hdr->page_size);
	dprintf(CRITICAL, "[%s] Android Hosd IMG Hdr - Command Line   : %s\n"    , MODULE_NAME, boot_hdr->cmdline);
	printf("============================================================\n");
	...
	//*
	if (strncmp((char const *)boot_hdr->magic,BOOT_MAGIC, sizeof(BOOT_MAGIC))!=0) {
		dprintf(CRITICAL, "[%s] hosd image header magic error\n", MODULE_NAME);
		return -1;
	}
	...
	if(len != -1)
	{
		...
		//***************
		//* specify hosd image size
		//*
		//g_bimg_sz = PART_BLKS_BOOTIMIG * BLK_SIZE;
		g_bimg_sz = (k_pg_cnt + r_pg_cnt + 1)* pg_sz;

		printf(" > hosd image size = 0x%x\n",g_bimg_sz);
	}
...
#if _MAKE_HTC_LK
/**********************************************************
 * Routine: mboot_android_load_hosdimg_hdr
 *
 * Description: this is the entry function to handle Hosd image header
 *
 **********************************************************/
int mboot_android_load_hosdimg_hdr(char *part_name, unsigned long addr)
{
	long len;
	//	unsigned long begin;
	//	unsigned long start_addr;
	...
	if (!dev)
	{
		printf("mboot_android_load_hosdimg_hdr, dev = NULL\n");
		return -ENODEV;
	}
	...
	if (!part)
	{
		printf("mboot_android_load_hosdimg_hdr (%s), part = NULL\n",part_name);
		return -ENOENT;
	}
	...
	if (!boot_hdr)
	{
		printf("mboot_android_load_hosdimg_hdr, boot_hdr = NULL\n");
		return -ENOMEM;
	}

	g_boot_hdr = boot_hdr;

	len = mboot_android_check_hosdimg_hdr(dev, part_name, boot_hdr);
...
#if _MAKE_HTC_LK
/**********************************************************
 * Routine: mboot_android_load_hosdimg
 *
 * Description: main function to load Android Hosd Image
 *
 **********************************************************/
int mboot_android_load_hosdimg(char *part_name, unsigned long addr)
{
	long len;
#ifdef MTK_EMMC_SUPPORT
	unsigned long long start_addr;
	...
	if (!dev)
	{
		printf("mboot_android_load_hosdimg , dev = NULL\n");
		return -ENODEV;
	}
	...
	if (!part)
	{
		printf("mboot_android_load_hosdimg , part = NULL\n");
		return -ENOENT;
	}
