...
static unsigned long rtc_boot_time = 0;
static bool rtc_boot_mode = false;
...
bool rtc_boot_check(bool can_alarm_boot)
{
	...
	if (irqsta & RTC_IRQ_STA_AL) {
		...
		if (pdn1 & RTC_PDN1_PWRON_TIME) {	/* power-on time is available */
			...
			rtc_boot_time = (time / 60) * 60; //rtc_mktime(yea, mth, dom, hou, min, 0);
			...
			dprintf(CRITICAL, "power-on = %d/%d/%d %d:%d:%d (%lu)(%lu)\n",
                               yea, mth, dom, hou, min, sec, time, rtc_boot_time);
			...
			if (now_time >= time - 1 && now_time <= time + 4) {	/* power on */
				...
				dprintf(CRITICAL,"RTC boot mode: %d, time: %d\n", g_boot_mode, rtc_boot_time);
				rtc_boot_mode = true;
				dprintf(CRITICAL,"RTC boot: %s  mode is :%d\n", rtc_boot_mode? "true" : "false" , g_boot_mode );
				return true;
...
bool rtc_alarm_check(unsigned long *rtc_time)
	{
		U16 now_sec, now_min, now_hou, now_dom, now_mth, now_yea;
		unsigned long now_time;
			...
			now_time = rtc_mktime(now_yea, now_mth, now_dom, now_hou, now_min, now_sec);
			rtc_boot_time = (now_time / 60) * 60; //rtc_mktime(yea, mth, dom, hou, min, 0);
			dprintf(CRITICAL, "now = %d/%d/%d %d:%d:%d (%lu)\n", now_yea, now_mth, now_dom, now_hou, now_min, now_sec, now_time);
		dprintf(CRITICAL,"RTC alarm check time: %d\n", rtc_boot_time);
		*rtc_time = rtc_boot_time;
		return 1; //rtc_boot_mode;
	}
